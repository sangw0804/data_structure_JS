...
class BTree {
  ...
  _swap(delNode, index, isSnapshot = false) {
  ...
  _borrowKey(parent, index, isSnapshot = false) {
  ...
  _bindNode(parent, index, isSnapshot = false) {
    let isLeft = true;
  ...
  remove(value, isSnapshot = false) {
    ...
    while (current) {
      ...
      if (parent && current.size() <= Math.floor(this.limit / 2)) {
        if (!this._borrowKey(parent, i, isSnapshot)) current = this._bindNode(parent, i, isSnapshot);
      }

      if (current.findValue(value) >= 0) {
        if (!current.leftChildOf(0)) break; // current 가 leaf 노드 인 경우
        value = this._swap(current, current.findValue(value), isSnapshot); // leaf 노드가 아니면 swap 한 다음, 다시 새로운 value를 지우는 알고리즘 실행
      }
